...
function generateAssessement() {
  ...
  $.post("http://127.0.0.1:5000/assessmentComplete", JSON.stringify(sendingData), function (response) {
    let breakPoint = 30
    document.querySelector(".chatBot").innerHTML = ""
    const resultSection = document.querySelector(".chatBot")
    console.log(response < breakPoint, response, typeof (breakPoint))
    if (response < breakPoint) {
      resultSection.innerHTML = `<h5>Congratulations, you've successfully killed Coronavirus. Have a look <a href="https://go.echoar.xyz/1kj2">here</a></h5>`
    } else {
      resultSection.innerHTML = `<h5>I'm afraid, the <a href="https://go.echoar.xyz/XKF9">Coronavirus</a> can kill you. But it can always be fought with the precautions mentioned in our Precautions page.</h5>`
    }
  })
}